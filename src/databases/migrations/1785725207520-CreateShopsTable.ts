...
export class CreateShopsTable1785725207520 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "shops" ADD "slug" character varying(255) NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "shops" ADD CONSTRAINT "UQ_8c28ec876676eeb1dcb65c01b7f" UNIQUE ("slug")`,
    );
    await queryRunner.query(`DROP INDEX "public"."ix_shops_status"`);
    await queryRunner.query(`ALTER TABLE "shops" DROP COLUMN "status"`);
    await queryRunner.query(
      `CREATE TYPE "public"."shop_status_enum" AS ENUM('pending', 'active', 'rejected', 'suspended')`,
    );
    await queryRunner.query(
      `ALTER TABLE "shops" ADD "status" "public"."shop_status_enum" NOT NULL DEFAULT 'pending'`,
    );
    await queryRunner.query(`ALTER TABLE "shops" DROP COLUMN "status"`);
    await queryRunner.query(
      `ALTER TABLE "shops" ADD "status" character varying(20) NOT NULL DEFAULT 'pending'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "shops" ADD CONSTRAINT "CHK_52244ebe7249e85b017f9ec0da" CHECK ("status" IN ('pending','active','rejected','suspended'))`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "ix_shops_status" ON "shops" USING btree ("status") `,
    );
    await queryRunner.query(
      `ALTER TABLE "shops" DROP CONSTRAINT "UQ_8c28ec876676eeb1dcb65c01b7f"`,
    );
